...
void mythread::run()
{
    ...
    while(runThread)    {
        ...
        if(!bl[ed_no])   {
            ...
            QString a2 = ("ping -w 1 10.50.0." + QString::number(array[ed_no]) +  " | sed -n '2p' | sed -e 's/Unreachable/0/' -e 's/ms/1/' | awk '{print $NF}' > /home/eds_cm/.eds_cs_files/log_eds_control_station/health_log.txt");
            char* c2 = new char[a2.length() + 1];
            strcpy(c2, a2.toLatin1().constData());
            system(c2);
            QFile dosya_ping("//home//eds_cm//.eds_cs_files//log_eds_control_station//health_log.txt");
            if(!dosya_ping.open(QIODevice::ReadOnly))
                QMessageBox::information(NULL,"info", dosya_ping.errorString());

            QTextStream in(&dosya_ping);
            in >> log_ping;
            dosya_ping.close();
            if(log_ping == "1")
                bl[ed_no] = false;
