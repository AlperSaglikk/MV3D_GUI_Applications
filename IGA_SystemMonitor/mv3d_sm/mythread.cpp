...
void mythread::run()
{
    QString firestarter = ("ssh 10.1.10." + QString::number(mv3d_no) + " ssh scc cat /opt/eds/log/diag/bit/bit_20* | grep -E 'S:DISCRETES|HVPS|FPGA|CHILLER|SYSTIC|Photocells|boardTemp|UPS' | grep -Ev ':ENCODER|:VIN' > /tmp/.sm_logs/bit_all");
    ...
    while(runThread) {
        QString secfirestarter = ("ssh 10.1.10." + QString::number(mv3d_no) + " ssh scc tail -1000 `ssh 10.1.10." + QString::number(mv3d_no) + " ssh scc ls -d -1 /opt/eds/log/diag/bit/bit_20* | tail -n 1` | grep -E 'S:DISCRETES|HVPS|FPGA|CHILLER|SYSTIC|Photocells|boardTemp|UPS' | grep -Ev ':ENCODER|:VIN' >  /tmp/.sm_logs/bit_all");
